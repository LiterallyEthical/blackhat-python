...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        ...
        epilog=textwrap.dedent(
            '''Example:
            spynet.py -t 192.168.1.108 -p 5555 -l -c # command shell
            spynet.py -t 192.168.1.108 -p 5555 -l -u=badfile.txt # upload to file
            spynet.py -t 192.168.1.108 -p 5555 -l -e=\"cat /etc/passwd\" # execute a command
            echo 'ABC' | ./spynet.py -t 192.168.1.108 -p 135 # echo text to server port 135
            spynet.py -t 192.168.1.108 -p 5555 # connect to server
        '''
        ))
    ...
    if args.listen:
        buffer = ''  # invoke the spynet object with an empty buffer string
    else:
        buffer = sys.stdin.read()

    sn = SpyNet(args, buffer.encode())  # Creating a SpyNet object
    sn.run()
    ...
    def __init__(self, args, buffer=None):
        ...
        def handle(self, client_socket):
            if self.args.execute:
                ...
                message = f'Saved file {self.args.uplaoad}'
